...
class SalesVisualizer(DataVisualizer):
    def visualize(self, data: dict):
        print(
            f"[SalesVisualizer] 📊 Drawing Bar Chart for Product: {data['product']} (Net: ${data['net_price']})"
        )
...
class FinanceVisualizer(DataVisualizer):
    def visualize(self, data: dict):
        print(
            f"[FinanceVisualizer] 📈 Drawing Trend Line Chart for Account: {data['account']} (Risk: {data['risk_level']})"
        )
...
class DataEngineeringPipeline:
    def __init__(
        self, cleaner: DataCleaner, processor: DataProcessor, visualizer: DataVisualizer
    ):
...
if __name__ == "__main__":
    ...
    sales_pipeline = DataEngineeringPipeline(
        SalesCleaner(), SalesProcessor(), SalesVisualizer()
    )
    ...
    finance_pipeline = DataEngineeringPipeline(
        FinanceCleaner(), FinanceProcessor(), FinanceVisualizer()
    )
