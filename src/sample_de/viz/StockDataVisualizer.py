...
class StockDataVisualizer(DataVisualizer):
    ...
    def candlestickChart(self, data: pd.DataFrame) -> None:
        ...
        chartfilePath = os.path.join(
            self.chartsPath, f"candle_chart_{self.langLocal}.png"
        )
    ...
    def advanceCandlestickChart(self, data: pd.DataFrame) -> None:
        ...
        chartfilePath = os.path.join(
            self.chartsPath, f"advanced_candle_chart_{self.langLocal}.png"
        )
        plots = [
            mpf.make_addplot(data["RSI"], panel=2, color="blue", ylabel="RSI (14)"),
            mpf.make_addplot(
                pd.Series(70, index=data.index), panel=2, color="gray", linestyle="--"
            ),
            mpf.make_addplot(
                pd.Series(30, index=data.index), panel=2, color="gray", linestyle="--"
            ),
