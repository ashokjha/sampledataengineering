...
class EcomSalesDataVisualizer(DataVisualizer):
    ...
    def monthlySalesTrendAndPriceDist(self, data: pd.DataFrame) -> None:
        ...
        trend_data = (
            data.groupby(["Month", "Subcategory"])["Total_Sales"].sum().reset_index()
        )
        sns.lineplot(
            ax=axes[0],
            data=trend_data,
        ...
        axes[0].set_title(
            self.translation.get_text(
                "Monthly_Sales_Trend_by_Subcategory", self.langLocal
            ),
            fontsize=14,
            fontweight="bold",
        )
        axes[0].set_xlabel(
            self.translation.get_text("Month_Timeline", self.langLocal), fontsize=12
        )
        axes[0].set_ylabel(
            self.translation.get_text("Total_Sales", self.langLocal), fontsize=12
        )
        axes[0].legend(
            title=self.translation.get_text("Subcategories", self.langLocal),
        ...
        axes[1].set_title(
            self.translation.get_text(
                "Price_Distribution_by_Category_and_Subcategory", self.langLocal
            ),
            fontsize=14,
            fontweight="bold",
        )
        axes[1].set_xlabel(
            self.translation.get_text("Category", self.langLocal)
            + " -> "
            + self.translation.get_text("SubCategory", self.langLocal),
            fontsize=12,
        )
        axes[1].set_ylabel(
            self.translation.get_text("Price_Per_Unit", self.langLocal), fontsize=12
        )

        # Rotate x-axis labels by 45 degrees so that text does not overlap and is clearly readable
        axes[1].set_xticklabels(axes[1].get_xticklabels(), rotation=45, ha="right")
        axes[1].legend(
            title=self.translation.get_text("Main_Categories", self.langLocal)
        )

        # Adjust layout to prevent overlapping
        plt.tight_layout()
        # Save the figure to a file
        chartPath = os.path.join(
            self.chartsPath, f"sales_visualizations_{self.langLocal}.png"
        )
    ...
    def revenueAndMonthlySalesTrend(self, data: pd.DataFrame) -> None:
        ...
        plt.title(
            self.translation.get_text("Revenue_by_Product_Category", self.langLocal),
            fontsize=14,
            fontweight="bold",
        )
        ...
        monthly_sales = (
            ...
            )  # Abbreviated Month Names. For full month names, use list(calendar.month_name)[1:]
            .reset_index()
        )
        ...
        plt.title(
            self.translation.get_text("Monthly_Sales_Trend", self.langLocal),
            fontsize=14,
            fontweight="bold",
        )
        plt.xlabel(self.translation.get_text("xlabel", self.langLocal), fontsize=12)
