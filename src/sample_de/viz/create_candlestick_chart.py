...
class candle_stick_chart:
    ...
    def create_candlestick_chart(self):
        ...
        stock_data = pd.read_csv(
            self.datasetpath, parse_dates=["Date"], index_col="Date"
        )
        ...
        chartPath = os.path.join(
            self.chartLocation, f"candle_chart_{self.langLocal}.png"
        )
        ...
        mpf.plot(
            ...
            title=self.translation.get_text(
                "Stock_Performance_Candlestick_Chart", self.langLocal
            ),
    ...
    def create_advanced_candlestick_chart(self):
        ...
        chartPath = os.path.join(
            self.chartLocation, f"advanced_candle_chart_{self.langLocal}.png"
        )
        plots = [
            mpf.make_addplot(
                stock_data["RSI"], panel=2, color="blue", ylabel="RSI (14)"
            ),
            mpf.make_addplot(
                pd.Series(70, index=stock_data.index),
                panel=2,
                color="gray",
                linestyle="--",
            ),
            mpf.make_addplot(
                pd.Series(30, index=stock_data.index),
                panel=2,
                color="gray",
                linestyle="--",
            ),
            ...
            mpf.make_addplot(
                stock_data["Signal"], panel=3, color="gray", linestyle="-"
            ),
            mpf.make_addplot(
